# https://github.com/deepseek-ai/DeepSeek-V3/blob/f09f5fa/inference/model.py
import math
...
def linear(
    ...
    Args:
        ...
        bias (Optional[torch.Tensor]): The bias tensor to be added.
            Default is None.
    ...
    Notes:
        - If `weight` is quantized (e.g., `element_size() == 1`), a dequantized
            version is used for computation.
        - If `gemm_impl == "bf16"`, dequantization and a `bf16` GEMM operation
            are applied.
...
class ColumnParallelLinear(Linear):
    ...
    def __init__(
        ...
        assert out_features % world_size == 0, (
                "Output features must be divisible by world size "
                f"(world_size={world_size})")
...
class RowParallelLinear(Linear):
    """
    Linear layer with row parallelism, splitting input features across
    distributed processes.
    ...
    def __init__(
        ...
        assert in_features % world_size == 0, (
                "Input features must be divisible by world size "
                f"(world_size={world_size})")
...
def precompute_freqs_cis(args: ModelArgs) -> torch.Tensor:
    """
    Precomputes frequency-based complex exponential values for rotary
...
class MLA(nn.Module):
    ...
    Attributes:
        ...
        n_heads (int): Number of attention heads.
        n_local_heads (int):
            Number of local attention heads for distributed systems.
        q_lora_rank (int): Rank for low-rank query projection.
        kv_lora_rank (int): Rank for low-rank key/value projection.
        qk_nope_head_dim (int):
            Dimensionality of non-positional query/key projections.
        qk_rope_head_dim (int):
            Dimensionality of rotary-positional query/key projections.
        qk_head_dim (int): Total dimensionality of query/key projections.
        v_head_dim (int): Dimensionality of value projections.
        softmax_scale (float):
            Scaling factor for softmax in attention computation.
    """
    def __init__(self, args: ModelArgs):
        ...
        if attn_impl == "naive":
            self.register_buffer("k_cache", torch.zeros(
                    args.max_batch_size, args.max_seq_len, self.n_local_heads,
    ...
    def forward(
        ...
        Args:
            ...
            mask (Optional[torch.Tensor]):
                Mask tensor to exclude certain positions from attention.
        ...
        if attn_impl == "naive":
            ...
            wkv_b = self.wkv_b.weight if self.wkv_b.scale is None else \
                    weight_dequant(
...
class Transformer(nn.Module):
    """
    Transformer model with positional embeddings, multiple layers, and output
    projection.
    ...
    def forward(self, tokens: torch.Tensor, start_pos: int = 0):
        ...
        Args:
            tokens (torch.Tensor):
                Input tensor of token IDs with shape (batch_size, seq_len).
            start_pos (int, optional):
                Starting position in the sequence for rotary embeddings.
                Defaults to 0.
